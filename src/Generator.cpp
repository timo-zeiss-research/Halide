...
Module GeneratorBase::build_gradient_module(const std::string &function_name) {
    ...
    // Dodge all the special cases for now
    for (const auto *input : pi.inputs()) {
        const size_t tuple_size = input->types_defined() ? input->types().size() : 1;
        internal_assert(tuple_size == 1) << "TODO(srj): Tuple Inputs are not yet tested";
    }
    for (const auto *output : pi.outputs()) {
        const size_t tuple_size = output->types_defined() ? output->types().size() : 1;
        internal_assert(tuple_size == 1) << "TODO(srj): Tuple Outputs are not yet tested";
    }
    ...
    for (const auto *input : pi.inputs()) {
        ...
        for (const auto &p : input->parameters_) {
            ...
            debug(1) << "    gradient copied input is: " << gradient_inputs.back().name << "\n";
        }
    }
    ...
    // - If an output's type is Tuple, each array element will have will have a separate input for each Tuple element.
    ...
    for (const auto *output : pi.outputs()) {
        for (size_t i = 0; i < output->funcs().size(); ++i) {
            ...
            const size_t tuple_size = output->types_defined() ? output->types().size() : 1;
            for (size_t t = 0; t < tuple_size; ++t) {
                const std::string output_name = output->array_name(i) + ((tuple_size > 1) ? ("." + std::to_string(t)) : "");
                // output_name is something like "funcname_i"   (for non-Tuples)
                //                            or "funcname_i.0" (for Tuples)
                ...
                const Type grad_in_type = output->types().at(t) == Float(64) ? Float(64) : Float(32);
                const int grad_in_dimensions = f.dimensions();
                const ArgumentEstimates grad_in_estimates = f.output_buffers().at(t).parameter().get_argument_estimates();
                ...
                debug(1) << "    gradient synthesized input is: " << gradient_inputs.back().name << "\n";
            }
        }
    }
    ...
    for (size_t i = 0; i < original_outputs.size(); ++i) {
        ...
        const std::string output_name = original_output.name();  // TODO(srj) something like output->array_name(i) + ((tuple_size > 1) ? ("." + std::to_string(t)) : "");
        for (const auto *input : pi.inputs()) {
            for (size_t i = 0; i < input->funcs_.size(); ++i) {
                ...
                debug(1) << "    gradient output is: " << d_out_wrt_in.name() << "\n";
            }
        }
    }
